...
- `horizon_days` (int) — горизонт прогноза в днях (например, 7, 14, 28)
...
3. Пользователь выбирает горизонт прогноза и нажимает "Запустить прогноз" - Forecast переходит в status=processing;
...
сказать про fine tuning - дообучение на новых данных. 
